...
def generate_spiral(num_points=3000, noise_std=0.02, num_turns=3):
    """
    Generate a 2D spiral pattern with added Gaussian noise.

    Args:
        num_points: Total number of points in the spiral.
        noise_std: Standard deviation of Gaussian noise added to the points.
    ...
    """
    theta = torch.linspace(0, 2 * np.pi * num_turns, num_points)  # Angle
    r = torch.linspace(0, 2.6, num_points)  # Radius
    x = r * torch.cos(theta)
    y = r * torch.sin(theta)
